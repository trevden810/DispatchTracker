...
export class GeographicCorrelationEngine {
  ...
  private async prepareJobLocations(jobs: Job[]): Promise<void> {
    ...
    for (const job of jobs) {
      if (!this.jobLocationCache.has(job.id)) {
        // Try address first, then customer as fallback
        const addressToGeocode = job.address || job.customer

        if (addressToGeocode) {
          try {
            console.log(`🔍 Attempting to geocode job ${job.id}: "${addressToGeocode}"`)
            ...
            if (geocoded) {
              ...
              console.log(`✅ Geocoded job ${job.id}: ${geocoded.lat}, ${geocoded.lng}`)
            } else {
              console.warn(`⚠️ No geocoding results for job ${job.id}: "${addressToGeocode}"`)
            }
          } catch (error) {
            console.warn(`❌ Geocoding error for job ${job.id}: ${addressToGeocode} - ${error}`)
          }
        } else {
          console.warn(`⚠️ Job ${job.id} has no address or customer data for geocoding`)
        }
      }
    }
