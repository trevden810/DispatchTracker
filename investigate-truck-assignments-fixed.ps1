...
try {
    ...
    Write-Host "  trucks_id: $($response.field_status.original_fields.trucks_id)" -ForegroundColor Gray
    ...
    try {
        # Fix the ampersand issue by using proper URL encoding
        $jobsUrl = "http://localhost:3002/api/jobs?active=true" + "&limit=3"
        $jobsResponse = Invoke-RestMethod -Uri $jobsUrl -Method GET -TimeoutSec 15
