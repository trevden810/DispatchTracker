# DispatchTracker: Test jobs_api_fleet Layout
...
foreach ($layout in $testLayouts) {
    try {
        ...
        if ($testResult.success -and $testResult.count -gt 0) {
            Write-Host "  ✅ $layout: $($testResult.count) jobs found" -ForegroundColor Green
        } else {
            Write-Host "  ⚠️ $layout: No jobs" -ForegroundColor Yellow
        }
    } catch {
        Write-Host "  ❌ $layout: Failed" -ForegroundColor Red
    }
}
